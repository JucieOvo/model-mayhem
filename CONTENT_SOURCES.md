...
| 公开发行文档中的唯一 URL 字符串 | 198 | 全部作为外部来源或公开资料链接保留 |
...
公开发行包不包含内部设计文档。通用内容卡的 `sources` 已改为发行包内实际存在的
`CONTENT_SOURCES.md` 或 `content/balance/standard-v0.1.yaml`。

当前内容包统计为 `archive=139`、`community=42`。139 张 `archive` 卡均至少包含一个可追溯
URL；此前关于“13 张 archive 卡只有内部设计来源”的说明已不再适用。
...
3. `archive` 卡虽然均有 URL，但仍需逐条复核来源是否足以支撑对应事实和分类，而不只检查链接存在。
